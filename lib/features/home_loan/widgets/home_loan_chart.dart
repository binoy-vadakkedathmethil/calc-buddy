...
class HomeLoanChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double principalPercentage =
        total == 0 ? 0 : principal / total;

    final double interestPercentage =
        total == 0 ? 0 : interest / total;
...
class _LoanChartPainter extends CustomPainter {
  ...
  void paint(
    ...
    // Principal
    paint.color = const Color(0xFF19A86B);

    canvas.drawArc(
      rect,
      startAngle,
      2 * pi * principalPercentage,
      false,
      paint,
    );

    // Interest
    paint.color = const Color(0xFFFD746C);

    canvas.drawArc(
      rect,
      startAngle +
          2 * pi * principalPercentage,
      2 * pi * interestPercentage,
      false,
      paint,
    );
  }
  ...
  @override
  bool shouldRepaint(
    covariant _LoanChartPainter oldDelegate,
  ) {
    return oldDelegate.principalPercentage !=
            principalPercentage ||
        oldDelegate.interestPercentage !=
            interestPercentage;
  }
}